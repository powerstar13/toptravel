...
<div class="container">
	<div class="page-header">
		<!-- 제목 -->
		<div class="pull-left">
			<h1>공연·전시(${perforItem.realmName})</h1>
		</div>
	...
	<div class="pull-right">
		...
		<button class="btn btn-primary" type="button"
			onclick="location.href='cultureFestival.do'">행사, 축제
		</button>
	<!-- 즐겨찾기, 좋아요 버튼  -->
	<c:choose>
	...
	<c:choose>
		...
		<small class="disinb"><i id="likeCnt" class="likeCnt">${perforItem.cultureLike}</i></small>
	...
	<table class="tbl_info table">
                    <!-- 공연기간,공연시간,공연가격,공연장소,주최,관람시간,연령 정보 제공 -->
                    <colgroup>
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                                 ...
                                 <c:choose>
                                  <c:when test="${perforItem.startDate <= perforItem.endDate}">
                                    ${perforItem.startDate}~${perforItem.endDate}
                                  </c:when>
                                  <c:otherwise>
                                  	담당자 문의 요망
                                  </c:otherwise>
                    ...
                    <tr>
                        <th scope="row">공연가격</th>
                        <td colspan="3">
                        	<c:if test="${perforItem.price != null}">
                        		${perforItem.price}
                        	</c:if>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                        <c:if test="${perforItem.area != null}">
                            	${perforItem.area}
                        </c:if>
                            <span class="txt">&nbsp;${perforItem.place}</span>
                        </td>
                        <th scope="row" class="rightLine">상세주소</th>

                        <td>
                        	<c:if test="${perforItem.placeAddr != null}">
                        		${perforItem.placeAddr}
                        	</c:if>
                        </td>
                    ...
                    <tr>
                        <th scope="row">연락처</th>
                        <td>
	                        <c:if test="${perforItem.phone != null}">
	                            	${perforItem.phone}
	                        </c:if>
                        </td>
                        <th scope="row">홈페이지</th>
                        <td>
	                        <c:if test="${perforItem.placeUrl != null}">
	                            	<a href="${perforItem.placeUrl}" target="_blank">${perforItem.placeUrl}</a>
	                        </c:if>
                        </td>
                    </tr>
                    </tbody>
    ...
    <div class="contents1">
      <c:if test="${perforItem.thumbnail != null}">
    	<div class="imageBox">
    		<img src="${perforItem.thumbnail}" />
    	</div>
      </c:if>
    	${perforItem.contents1}
	</div>
	...
	<div class="listBtn">
		<a class="btn btn-primary goList text-center" href="${pageContext.request.contextPath}/culture/culturePerformance.do">
		목록보기
		</a>
	</div>
...
<!-- 개인 js 참조 영역 -->
<script>
	var cultureId = "${perforItem.seq}";
    var lng = "${perforItem.gpsX}";
    var lat = "${perforItem.gpsY}";
    var place = "${perforItem.place}";
</script>
  ...
  //게시글 좋아요
    $(document).on("click", ".btn-like", function() {
        ...
        if (target.find(".far").length == 1) {
            ...
            $.ajax({
                method: "post",
                url: ROOT_URL + "/culture/culture_like.do",
                data: {
                    cultureId: cultureId,
