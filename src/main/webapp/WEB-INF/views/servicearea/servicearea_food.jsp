...
<div class="row clearfix">
    ...
    <article class="col-md-10 pull-left">
        ...
        <!-- 노선 검색 폼 -->
        <form action="${pageContext.request.contextPath}/servicearea/servicearea_food.do" method="GET" id="line_search" class="form-horizontal" role="form">
            <fieldset>
...
<c:choose>
    <c:when test="${fn: length(lineList) > 0}">
        ...
        <c:forEach var="item" items="${lineList}">
            <li>
                <a href="${pageContext.request.contextPath}/servicearea/servicearea_food_result.do?routeName=${item.routeName}">
                    <span class="icon">
...
<c:choose>
    <c:when test="${pageHelper.prevPage > 0}">
        ...
        <c:url var="prevUrl" value="/servicearea/servicearea_food.do">
...
<c:forEach var="i" begin="${pageHelper.startPage}" end="${pageHelper.endPage}" varStatus="status">
    ...
    <c:url var="pageUrl" value="/servicearea/servicearea_food.do">
        <c:param name="lineName" value="${lineName}" />
...
<c:choose>
    <c:when test="${pageHelper.nextPage > 0}">
        ...
        <c:url var="nextUrl" value="/servicearea/servicearea_food.do">
            <c:param name="lineName" value="${lineName}" />
