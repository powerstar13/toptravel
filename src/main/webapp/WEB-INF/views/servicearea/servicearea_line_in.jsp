...
<c:choose>
    ...
    <c:otherwise>
                <%-- 조회결과가 존재하지 않는 경우 --%>
                <tr class="text-center">
                    <td colspan="2">조회된 휴게소 목록이 없습니다.</td>
                </tr>
    </c:otherwise>
</c:choose>
            </tbody>
...
<c:choose>
    <c:when test="${pageHelper.prevPage > 0}">
        ...
        <c:url var="prevUrl" value="/servicearea/servicearea_line_in.do">
...
<c:forEach var="i" begin="${pageHelper.startPage}" end="${pageHelper.endPage}" varStatus="status">
    ...
    <c:url var="pageUrl" value="/servicearea/servicearea_line_in.do">
        <c:param name="routeName" value="${routeName}" />
...
<c:choose>
    <c:when test="${pageHelper.nextPage > 0}">
        ...
        <c:url var="nextUrl" value="/servicearea/servicearea_line_in.do">
            <c:param name="routeName" value="${routeName}" />
...
<c:if test="${fn: length(groupList) > 0}">
    ...
    --%>
        ...
        var serviceareaName = [];
        var placeId = [];
        var groupList_length = "${fn: length(groupList)}";
    <c:forEach var="item" items="${groupList}" varStatus="status">
        lng.push(${item.placeX});
        lat.push(${item.placeY});
        serviceareaName.push("${item.serviceareaName}");
        placeId.push("${item.placeId}");
    </c:forEach>
</c:if>
    </script>
    ...
    <script src="${pageContext.request.contextPath}/assets/js/servicearea_line_in.js"></script>
</body>
</html>
